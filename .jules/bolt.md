...
## 2024-06-09 - Optimize ordered array lookups in high-frequency event handlers
**Learning:** React components executing inside high-frequency event handlers (like `<video onTimeUpdate>`) suffer performance hits when using O(N) linear iteration over large ordered arrays (like timestamps) on every tick.
**Action:** Use O(log N) algorithms like binary search when looking up values in large ordered arrays during high-frequency events to maintain performance without blocking the main thread.

...
## 2024-05-25 - O(log N) lookup in high-frequency event handlers
**Learning:** High-frequency event handlers like `<video onTimeUpdate>` executing O(N) linear scans on large ordered arrays (like timestamps) can cause noticeable performance degradation and block the main thread.
**Action:** Use an O(log N) algorithm like binary search for looking up values in ordered arrays during high-frequency events to maintain performance without blocking the main thread.

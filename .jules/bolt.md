...
## 2024-06-15 - Optimize timestamp lookup in React video playback
**Learning:** React components executing inside high-frequency event handlers (like `<video onTimeUpdate>`) suffer from O(N) linear iteration when looking up values in large ordered arrays (like timestamps), which can block the main thread.
**Action:** Use O(log N) algorithms (e.g., binary search) rather than O(N) linear iteration when searching large ordered arrays in high-frequency event handlers to maintain performance.

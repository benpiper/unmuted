...
## 2024-06-15 - Optimize array lookups in React event handlers
**Learning:** High-frequency event handlers (like `<video onTimeUpdate>`) can cause performance issues if they use O(N) linear iteration over large arrays (like parsed timestamps).
**Action:** Use O(log N) binary search instead of O(N) iteration when looking up values in ordered arrays within high-frequency handlers to maintain performance without blocking the main thread.

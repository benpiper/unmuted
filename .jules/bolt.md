...
## 2024-05-25 - Optimize high-frequency event handlers with binary search
**Learning:** Linear O(N) searches in frequent events like React's `<video onTimeUpdate>` can block the main thread, especially when iterating over large ordered arrays of timestamps.
**Action:** Use O(log N) binary search instead of linear iteration when looking up values in large ordered arrays to maintain performance without blocking the main thread.

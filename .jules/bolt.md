...
## 2025-02-12 - Non-blocking sleep in FastAPI async routes
**Learning:** Using `time.sleep()` inside an `async def` route (e.g. while waiting for a file to exist) blocks the entire FastAPI event loop, severely degrading performance and responsiveness for all users under load.
**Action:** Always use `await asyncio.sleep()` when pausing execution in an `async` context to allow other tasks to run.

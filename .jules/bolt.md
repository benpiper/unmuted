...
## 2024-06-12 - Event Loop Blocking in FastAPI Polling
**Learning:** Using synchronous `time.sleep()` inside an `async def` route (like polling for a file) blocks the entire ASGI event loop, preventing the server from handling any other concurrent requests.
**Action:** Always use `await asyncio.sleep()` in async functions.

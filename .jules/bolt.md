...
## 2024-05-27 - Never block async event loop
**Learning:** Using synchronous `time.sleep()` inside an `async def` FastAPI route blocks the entire asyncio event loop, causing all other concurrent requests to stall until the sleep finishes.
**Action:** Always use `await asyncio.sleep()` for sleeping inside `async def` functions so that other tasks can continue processing.

...
## 2024-05-06 - Optimize SQLAlchemy bulk inserts
**Learning:** In SQLAlchemy, `Session.add_all()` (or looping `Session.add()`) still issues individual INSERT statements when flushed, incurring significant ORM overhead and N database round-trips.
**Action:** Use `await db.execute(insert(Model), list_of_mappings)` to perform a true bulk insert, reducing database round-trips to O(1) and avoiding ORM instantiation overhead.

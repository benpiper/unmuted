...
## 2024-06-08 - O(N) operations in onTimeUpdate
**Learning:** High-frequency event handlers like React `<video onTimeUpdate>` fire multiple times per second and can block the main thread if they contain O(N) linear scans over large arrays (like transcript timestamps).
**Action:** Always use O(log N) binary search or O(1) hash maps for lookups inside high-frequency event handlers instead of linear iteration.

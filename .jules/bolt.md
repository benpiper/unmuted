...
## 2024-05-24 - Optimize timestamp lookup in video playback
**Learning:** High-frequency event handlers like `<video onTimeUpdate>` executing O(N) linear searches on large ordered arrays can cause unnecessary CPU load and potential frame drops.
**Action:** Replace linear iterations with O(log N) binary search algorithms when looking up values in ordered arrays during high-frequency events to maintain performance without blocking the main thread.

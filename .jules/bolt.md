...
## 2024-06-25 - Use Binary Search for Ordered Lookups in High-Frequency Handlers
**Learning:** Linear scans (`O(N)`) inside high-frequency event handlers like `<video onTimeUpdate>` become a bottleneck as the dataset (e.g., transcript segments) grows.
**Action:** Replace `O(N)` iterative loops with `O(log N)` binary search algorithms when looking up values in large ordered arrays (like timestamps) to maintain performance without blocking the main thread.

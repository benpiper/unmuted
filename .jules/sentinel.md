...
## 2024-05-01 - [MEDIUM] Information Leakage in extract_project endpoint
**Vulnerability:** The `/api/project/extract` endpoint in `backend/main.py` directly returned `str(e)` in an `HTTPException` upon failure, which exposes internal exception strings or stack trace information to the client.
**Learning:** Directly passing exception strings to users in generic error handlers leaks internal implementation details. While FastAPIs error handler intercepts 500s, returning explicit 400 responses with the exception body bypasses that protection.
**Prevention:** To prevent information leakage in FastAPI backends, avoid returning raw exception details directly in `HTTPException` responses. Instead, catch the exception, log the full details securely (e.g. `logger.error(..., exc_info=True)`), and return a sanitized, generic 'Internal server error' message (with an HTTP 500 status code) to the client. Ensure intentional `HTTPException`s are explicitly caught and re-raised before the generic catch-all.

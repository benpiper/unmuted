...
def get_cors_origins():
    """Build CORS origins list, auto-detecting Render deployments and validating URLs."""
    import urllib.parse
    raw_origins = []
    ...
    for origin in raw_origins:
        # Wildcards are not allowed with allow_credentials=True and are a security risk
        if origin == "*":
            continue
...
async def save_project(req: SaveRequest, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        if project:
            ...
            # Add new segments
            # ⚡ Bolt: Bulk insert optimization to minimize database round-trips and ORM overhead
            segment_mappings = [
                {
                    "project_id": project.id,
            ...
            if segment_mappings:
                await db.execute(insert(TranscriptSegment), segment_mappings)
